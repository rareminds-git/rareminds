...
const GeneralEventDetail = () => {
  ...
  useEffect(() => {
    async function fetchEvent() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}generalEvents/${slug}`);
  ...
  return (
    <>
      <Helmet>
        ...
        />
        <meta name="keywords" content={eventData?.generalEventData?.MetaKeywords} />
        <meta property="og:title" content={eventData?.generalEventData?.OGTitle} />
        <meta
