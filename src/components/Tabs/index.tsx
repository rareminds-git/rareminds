...
const CustomTabs = ({ testimonials, tabs, categories }) => {
  return (
    <Tabs
      onSelect={() => window.scrollTo({ top: 0, behavior: "smooth" })} // Prevents scrolling to tabs
      className="border-0 relative pb-[50px]"
    >
      <TabList className="border-0 border-r-0 custom-tabs md:mx-0 absolute bottom-0">
        {tabs?.map((title: any) => {
          ...
          const testimonialData = testimonials.filter(
            (data) =>
              category.length > 0 && data.ContentId === category[0].ContentId,
          );
