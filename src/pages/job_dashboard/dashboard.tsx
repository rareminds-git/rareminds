...
import AnalyticsPage from "../job_dashboard/analytics";
...
const CandidatesDashboard = () => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && visibleCandidates.length < candidates.length) {
          loadMore();
        }
      },
      { threshold: 1.0 }
  ...
  const loadMore = () => {
    if (visibleCandidates.length < filteredCandidates.length) {
      const nextBatch = filteredCandidates.slice(visibleCandidates.length, visibleCandidates.length + PAGE_SIZE);
  ...
  const paginatedCandidates = filteredCandidates.slice(0, visibleCandidates.length);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <h1 className="text-4xl font-bold mt-2">
       {activeTab === "dashboard" ? (
       <>
        Empowering <span className="italic text-red-500"> Placements</span>
        <span className="block">Tracking Talent</span>
      </>
      ) : (
      <>
        Insights That <span className="italic text-red-500">Drive Success</span>
      </>
    )}
  </h1>
</div>
          ...
          {activeTab === "dashboard" ? (
            <div className="mb-6">
                    ...
                    {paginatedCandidates.length > 0 ? (
                      paginatedCandidates.map((candidate) => (
                        <tr key={candidate.id} className="border">
                          <td className="border p-2">{candidate.Candidate_ID || "N/A"}</td>
                          <td className="border p-2">{candidate.Full_Name}</td>
                          <td className="border p-2">{candidate.Email || "N/A"}</td>
                          <td className="border p-2">{candidate.Position_Hiring_for || "N/A"}</td>
                          <td className="border p-2">{candidate.Created_Time || "N/A"}</td>
                          <td className="border p-2">{candidate.Candidate_Status || "Unknown"}</td>
                          <td className="border p-2">{candidate.Experience_in_Years || "Unknown"}</td>
                          <td className="border p-2">{candidate.Highest_Qualification_Held || "Unknown"}</td>
                          <td className="border p-2">{candidate.Skill_Set || "Unknown"}</td>
                          <td className="border p-2">{candidate.City || "Unknown"}</td>
                          <td className="border p-2">{candidate.State || "Unknown"}</td>
                          <td className="border p-2">{candidate.Source || "Unknown"}</td>
                          <td className="border p-2">{candidate.Country || "Unknown"}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan="13" className="text-center p-2">No candidates found</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div ref={loaderRef} className="py-4 flex justify-center">
                {visibleCandidates.length < candidates.length && <PhotoLoader />}
              </div>
            </div>
          ) : (
