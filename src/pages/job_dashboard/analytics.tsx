...
const COLORS = ["#ff6666", "#ff3333", "#cc0000", "#6699ff", "#3366ff", "#0033cc"];
...
const AnalyticsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:6069/api/Candidates");
  ...
  const candidatesPreviousMonth = candidates.filter((candidate) => {
    const appliedDate = candidate.Created_Time ? dayjs(candidate.Created_Time) : null;
    return appliedDate && appliedDate.year() === previousYear && appliedDate.month() === previousMonth;
  }).length;

  const candidatesThisMonth = candidates.filter((candidate) => {
    const appliedDate = candidate.Created_Time ? dayjs(candidate.Created_Time) : null;
    return appliedDate && appliedDate.year() === currentYear && appliedDate.month() === currentMonth;
  }).length;
  ...
  const applicationTrends = [
    ...
    return { totalCandidates, candidatesThisMonth, statusData, cityData, positionData ,applicationTrends };
  };
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
          ...
          <div className="p-6 bg-white shadow-lg rounded-lg flex flex-col items-center text-center">
            <h3 className="text-xl font-semibold mb-2">Candidates This Month</h3>
            <motion.p
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="p-8 bg-white shadow-lg rounded-lg flex flex-col items-center text-center">
            <h3 className="text-xl font-semibold mb-4">Candidates by Position</h3>
            <ResponsiveContainer width="100%" height={400}>
        ...
        <div className="p-8 bg-white shadow-lg rounded-lg flex flex-col items-center text-center">
          <h3 className="text-xl font-semibold mb-4">Application Trends (Previous vs. Current Month)</h3>
          <ResponsiveContainer width="100%" height={400}>
