...
const stats = [
  {
    ...
    value: 10000, // Change to a number for easier manipulation
    subtext: '143 colleges/batches',
    icon: Users,
  },
  {
    label: '6th Semester Students',
    value: 20500, // Change to a number for easier manipulation
    subtext: '546 colleges/batches',
    icon: GraduationCap,
  },
  {
    label: 'Expert Trainers',
    value: 250, // Change to a number for easier manipulation
...
function useCounter(targetValue, duration) {
  ...
  useEffect(() => {
    ...
    const increment = Math.ceil(end / (duration / 1000 * 60)); // Calculate increment based on duration
...
export function AchievementsSection(s_data) {
  return (
      ...
      <div className="mx-auto max-w-7xl px-10 sm:px-8">
        ...
        >
          <div className="text-center">
            ...
            >
             Since 2024, we have made a significant impact by:
          
            </motion.p>
          </div>
          <dl className="mt-16 grid grid-cols-1 gap-8 sm:mt-12 sm:grid-cols-3">
            {stats.map((stat, index) => {
              const count = useCounter(stat.value, 2000); // 2000 ms for the animation duration
              return (
                <motion.div
                  key={stat.label}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  ...
                  <dd className="mt-4 flex flex-col items-center gap-1">
                    ...
                    <span className="text-lg font-semibold text-gray-900">{stat.label}</span>                    <span className="text-lg font-semibold text-gray-900">{stat.label}</span>
